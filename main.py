...
class CaloriesCalculator(Calculator):
    def get_calories_remained(self):  # Получает остаток калорий на сегодня
        ...
        if x > 0:
            # Здесь можно сократить запись ветвления: "if x:" и можно по сути убрать else ниже, он не требуется
...
class CashCalculator(Calculator):
    ...
    def get_today_cash_remained(self, currency,
        ...
        if currency == 'usd':
            ...
            currency_type = 'руб' # ниже желательно оставить пустую строку чтобы визуально разделить кейсы ветвления
        if cash_remained > 0:
            return (
